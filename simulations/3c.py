...
#parser.add_argument('-g', type=float, default=1.4, help='multiplier of gamma')
...
param_t=pd.DataFrame({'n': 6*[args.n], 'rho': 6*[args.r],
    'gamma' :[0, .4, .8, 1.2, 1.6, 2], 'seed': 6*[args. seed]})

results = Parallel(n_jobs=6) (delayed(solve) (row) for index, row in param_t.iterrows())
